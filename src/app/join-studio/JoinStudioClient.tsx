...
function StudioSignupForm({ brandPrimary, brandPrimaryHover }: StudioSignupFormProps) {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>): Promise<void> => {
    ...
    try {
      setSubmitting(true);
      await new Promise<void>((resolve) => setTimeout(resolve, 1500)); // Placeholder request
  ...
  return (
    <form ref={formRef} onSubmit={handleSubmit} className="space-y-6">
      ...
      <div className="space-y-2">
        ...
        <Input name="business-name" placeholder="Studio or business name" value={businessName} onChange={(e) => setBusinessName(e.target.value)} required className="w-full" />
      </div>

      <div className="space-y-2">
        <label className="block text-sm font-medium text-gray-700">Contact name</label>
        <Input name="contact-name" placeholder="Your full name" value={contactName} onChange={(e) => setContactName(e.target.value)} required className="w-full" />
      </div>

      <div className="space-y-2">
        <label className="block text-sm font-medium text-gray-700">Contact email</label>
        <Input type="email" name="email" placeholder="studio@example.com" value={email} onChange={(e) => setEmail(e.target.value)} required className="w-full" />
      </div>

      <div className="space-y-2">
        <label className="block text-sm font-medium text-gray-700">Contact number (optional)</label>
        <Input type="tel" name="phone" placeholder="+44 7123 456789 (optional)" value={phone} onChange={(e) => setPhone(e.target.value)} className="w-full" />
      </div>

      <div className="space-y-2">
        <label className="block text-sm font-medium text-gray-700">Location</label>
        <Input name="location" placeholder="London area (e.g., Peckham)" value={location} onChange={(e) => setLocation(e.target.value)} required className="w-full" />
      </div>

      <div className="space-y-2">
        <label className="block text-sm font-medium text-gray-700">Booking software (if any)</label>
        <Input name="booking-software" placeholder="Mindbody, WellnessLiving, None, etc." value={bookingSoftware} onChange={(e) => setBookingSoftware(e.target.value)} className="w-full" />
      </div>
      ...
      <div className="space-y-2">
        ...
        <textarea
          ...
          style={{ borderColor: "#2F7A80", outlineColor: "#2F7A80" }}
        />
      </div>
      ...
      <div className="pt-2">
        <Button
          ...
          style={{ backgroundColor: "#2F7A80" }}
          onMouseEnter={(e) => (e.currentTarget.style.backgroundColor = "#2A6C71")}
          onMouseLeave={(e) => (e.currentTarget.style.backgroundColor = "#2F7A80")}
        >
...
const FAQItem = ({ question, answer, brandPrimary }: FAQItemProps) => {
  ...
  return (
    <div className="border-b border-gray-200 last:border-b-0">
      <button className="flex justify-between items-center w-full py-4 text-left font-semibold text-gray-900 hover:text-gray-700 transition-colors" onClick={() => setIsOpen(!isOpen)} aria-expanded={isOpen}>
        <span>{question}</span>
        {isOpen ? <Minus size={20} style={{ color: brandPrimary }} /> : <Plus size={20} style={{ color: brandPrimary }} />}
      </button>
...
export default function JoinStudioClient() {
  ...
  const faqData = [
    { q: "What is Dabble and why partner?", a: "Dabble is a small London pilot (MVP) that helps curious locals discover in-person hobby classes — pottery, cooking, dance, improv and more. For the pilot we are offering partner studios free marketing and no commission so you can test whether Dabble drives bookings for you." },
    { q: "Is there any cost or commission?", a: "No — during the MVP/pilot period there are no fees and no commission. You keep your full class income. We will be transparent about future pricing if and when we launch a paid model, but for the pilot it is free for partners." },
    { q: "How do bookings arrive at my studio?", a: "We will list your class with your contact or booking link or enable direct bookings. Initially we can forward bookings to your email or phone, or integrate with a simple calendar or CSV flow depending on what works for you. We will discuss operational preferences during onboarding." },
    { q: "Who owns customer data?", a: "During the pilot we will share booking details you need to run the class (name, email, booking time). We do not keep this data private from you — we aim for a collaborative pilot. Longer-term policies (data ownership, export, GDPR) will be documented and agreed before any paid model." },
    { q: "What about cancellations or no-shows?", a: "Your studio can keep your usual cancellation and no-show policies. We will display the studio policy on the listing. If you would like, we can also include automated reminders to reduce no-shows." },
    { q: "What do you need from us to get listed?", a: "A contact email, short class description, price, dates and times or recurring schedule, a photo or logo, and your cancellation policy. That is it — we will create the listing for you and send a preview before it goes live." },
    { q: "How long is the pilot and what happens after?", a: "The pilot will run until we have enough data and partner feedback. We will confirm timelines with partners. After the pilot we may introduce a commercial model; we will discuss and agree terms with partner studios before rolling out any paid features." },
  ];

  return (
<div className="min-h-screen bg-gradient-to-b from-white via-slate-50 to-pink-50 text-gray-900">
    <Script src="https://challenges.cloudflare.com/turnstile/v0/api.js" async defer />

      {/* HERO SECTION */}
      <header className="relative mt-0 h-[60vh] min-h-[400px]">
        <div className="absolute inset-0 w-full h-full bg-cover bg-center" style={{ backgroundImage: `url('${heroBg}')` }} />
        <div className="absolute inset-0 bg-gradient-to-t from-black/60 via-black/30 to-transparent" />
        <div className="relative z-10 w-full h-full flex flex-col items-center justify-center px-4 text-center">
          <h1 className="text-white text-4xl md:text-5xl font-bold max-w-4xl mx-auto drop-shadow-lg" style={{ fontFamily: "'Poppins', sans-serif" }}>
            Partner with Dabble:
            <br />
      ...
      <main className="max-w-6xl mx-auto px-6 mt-16 space-y-20">
        ...
        <section className="bg-white py-12 rounded-2xl shadow-xl border border-gray-100">
          <div className="text-center">
            ...
            <p className="mt-2 text-lg text-gray-600">Grow your studio with pre-paid, engaged new customers.</p>
          </div>
        ...
        <section className="py-12">
          <div className="text-center">
            ...
            <p className="mt-2 text-lg text-gray-600">Find quick answers to common partner questions about the pilot.</p>
          </div>
          <div className="max-w-4xl mx-auto mt-10 bg-white rounded-2xl shadow-lg border border-gray-100 divide-y divide-gray-200 p-6">
            {faqData.map((item, index) => (
              <FAQItem key={index} question={item.q} answer={item.a} brandPrimary={brandPrimary} />
            ))}
          </div>
        </section>
      </main>
      ...
      <footer className="border-t mt-10">
        <div className="max-w-6xl mx-auto px-6 py-8 text-sm text-gray-600 flex flex-col md:flex-row items-center justify-between gap-4">
          ...
          <div className="flex gap-4">
            ...
            <Link href="/newsletter-signup" className="hover:text-gray-900">Get updates</Link>
